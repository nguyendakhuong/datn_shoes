...
const Statistical = () => {
  ...
  const getStatistical = async () => {
    try {
      const res = await fetch(
        `http://localhost:3001/statistical/getInfoStatistical`,
        {
          headers: {
  ...
  const ordersByPaymentMethod = async () => {
    try {
      const res = await fetch(
        `http://localhost:3001/statistical/ordersByPaymentMethod`,
        {
          headers: {
            Authorization: `Bearer `,
          },
  ...
  const productSales = async () => {
    try {
      const res = await fetch(
        `http://localhost:3001/statistical/productSales`,
        {
          headers: {
            Authorization: `Bearer `,
          },
  ...
  const productSalesByDate = async () => {
    try {
      const res = await fetch(
        `http://localhost:3001/statistical/productSalesByDate`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer `,
  ...
  const getRevenueByMonth = async () => {
    ...
    if (Number(selectedYear) > nowYear) {
      ToastApp.warning("Không được chọn năm lớn hơn năm hiện tại");
      return;
    }
    try {
      const res = await fetch(
        `http://localhost:3001/statistical/revenueByMonth`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer `,
      ...
      if (data.status === 200) {
        ...
        ToastApp.warning("Lỗi lấy dữ liệu doanh thu");
      }
    } catch (error) {
      console.log("Lỗi lấy dữ liệu doanh thu theo tháng: ", error);
    }
  };
  useEffect(() => {
    getStatistical();
    ...
    <div className="statistical-container">
      ...
      <div className="charts">
        ...
        <div className="chart">
          ...
          <BarChart
            width={700}
            height={300}
            data={productDataTime.length > 0 ? productDataTime : productData}
          >
      ...
      <div className="chart">
        ...
        {revenueData && revenueData.length > 0 && (
          <BarChart width={1000} height={300} data={revenueData}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="month" />
            <YAxis />
            <Tooltip />
