...
const ModalDetails = ({ id, onClose, isOpen }) => {
  ...
  const handleChangeColor = async (selectedOption) => {
    ...
    if (selectedOption.__isNew__) {
      ...
      try {
        const response = await fetch(
          `http://localhost:3001/color/createColor`,
          {
            method: "POST",
            headers: {
  ...
  const handleClickStatus = async (e, code) => {
    try {
      const response = await fetch(
        `http://localhost:3001/product/statusProductDetail/${code}`,
        {
          headers: {
            Authorization: `Bearer `,
          },
      ...
      if (data.status === 200) {
        ...
        console.log(
          "Lỗi cập nhật trạng thái sản phẩm chi tiết :",
          data.message
        );
    ...
    dispatch({
      ...
      payload: {
        ...
        onClickConfirmModel: async (
          ...
          try {
            const response = await fetch(
              `http://localhost:3001/product/updateProductDetail`,
              {
                method: "PUT",
                headers: {
                  Authorization: `Bearer ${token}`,
  ...
  const handleSubmit = async (id) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/product/createProductDetail/${id}`,
        {
          method: "POST",
          headers: {
            // "Content-Type": "application/json",
            ...
            <div>
              <div className="dialog-content">
                <div className="info-container">
                  <p>
                    <strong>Mã sản phẩm:</strong> {data.code}
                  </p>
                  <p>
                    <strong>Tên sản phẩm:</strong> {data.name}
                  </p>
                  <p>
                    <strong>Thương hiệu:</strong> {data.trademark}
                  </p>
                  <p>
                    <strong>Suất xứ:</strong> {data.origin}
                  </p>
                  <p>
                ...
                <p>
                  <strong>Mô tải: </strong> {data.description}
                </p>
                <p>
                  ...
                  <div className="input-colorDetail">
                    ...
                    <div className="item_size input-container">
                      ...
                      >
                        ...
                        {Array.from({ length: 21 }, (_, i) => 30 + i).map(
                          (size) => (
                    ...
                    <div className="file_inputs">
                      <label htmlFor="file-upload" className="btn_image">
                        {imagePreview ? "Chọn ảnh khác" : "Thêm ảnh"}
                      </label>
                      <input
                        id="file-upload"
                        type="file"
