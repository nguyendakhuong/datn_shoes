  ...
  const getOrderNote = async () => {
    try {
      const response = await fetch(
        `http://localhost:3001/order/getOrderNote/${order.orderCode}`,
        {
          method: "GET",
          headers: {
    ...
    dispatch({
      ...
      payload: {
        ...
        onClickConfirmModel: async () => {
          try {
            const response = await fetch(
              `http://localhost:3001/order/verifyOrder/${orderCode}`,
              {
                method: "GET",
                headers: {
    ...
    dispatch({
      ...
      payload: {
        ...
        onClickConfirmModel: async () => {
          try {
            const response = await fetch(
              `http://localhost:3001/order/deliveryOrder`,
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
    ...
    dispatch({
      ...
      payload: {
        ...
        onClickConfirmModel: async (title, content, fileImage) => {
          ...
          try {
            const response = await fetch(
              `http://localhost:3001/order/confirmOrderByAdmin`,
              {
                method: "POST",
                headers: {
                  Authorization: `Bearer ${token}`,
    ...
    dispatch({
      ...
      payload: {
        ...
        onClickConfirmModel: async (title, content) => {
          ...
          try {
            const response = await fetch(
              `http://localhost:3001/order/cancelOrderAdmin`,
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
          ...
          <>
            <div className="modal1-content">
              <div className="left-content">
                {data.map((item, index) => (
                  <div key={index} className="item-container">
                    ...
                    <div className="info">
                      <p>
                        <strong>Mã đơn sản phẩm: {item.orderDetailCode}</strong>
                      </p>
                      <p>
                      ...
                      <p>
                        Giá<strong> {formatter.format(item.price)}</strong>
                      </p>
                    </div>
                  </div>
                ))}
              ...
              <div className="right-content">
                ...
                <p className="order-info">
                  <strong>Mã người dùng:</strong> {order.customerCode}
                </p>
                <p className="order-info">
                  <strong>Tên:</strong> {order.userName}
                </p>
                <p className="order-info">
                ...
                {orderNote && order.status === "6" ? (
                  <div>
                    ...
                    <p className="order-info">
                      <strong>Lí do hủy:</strong> {orderNote.content}
                    </p>
                  </div>
                ) : null}
                {orderNote && order.status === "5" ? (
                  <div className="flex">
                    <div>
                      ...
                      <p className="order-info">
                        <strong>Nội dung:</strong> {orderNote.content}
                      </p>
                    </div>
                    <div>
            ...
            <div className="footer">
              ...
              {order.status === "2" ? (
                <button
                  className="btn-btn"
                  onClick={() => handleDeliveryOrder(order.orderCode)}
                >
                  Giao hàng
                </button>
              ) : null}
