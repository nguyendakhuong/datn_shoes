...
const Cart = () => {
      ...
      if (data.status === 200) {
        ...
        setQuantities((prevQuantities) => {
          ...
          data.data.forEach((item) => {
            newQuantities[item.productDetailCode] =
              newQuantities[item.productDetailCode] || 1;
          });
  ...
  const increase = (productDetailCode, maxQuantity) => {
    setQuantities((prev) => ({
      ...prev,
      [productDetailCode]: Math.min(
        (prev[productDetailCode] || 1) + 1,
        maxQuantity
      ),
    }));
  };
  ...
  const decrease = (productDetailCode) => {
    setQuantities((prev) => ({
      ...prev,
      [productDetailCode]:
        prev[productDetailCode] > 1 ? prev[productDetailCode] - 1 : 1,
    }));
  };
  const handleUpdateQuantity = (productDetailCode) => {
    const item = data.find((p) => p.productDetailCode === productDetailCode);
    if (!item) return;
    dispatch({
      type: KEY_CONTEXT_USER.SHOW_MODAL,
      payload: {
        typeModal: "EDIT_ORDER_PRODUCT",
        titleModel: "Cập nhật số lượng sản phẩm mua !",
        onClickConfirmModel: async (newQuantity, listError) => {
          const parsedQuantity = Number(newQuantity);
          if (listError.quantity) {
            return ToastApp.warning("Dữ liệu không đúng định dạng");
          }
          if (parsedQuantity <= 0) {
          ...
          if (parsedQuantity > item.quantity) {
            ...
            return ToastApp.warning(
              `Số lượng còn lại của sản phẩm không đủ`
            );
          }
          setQuantities((prev) => ({
            ...prev,
            [productDetailCode]: parsedQuantity,
          }));
          dispatch({ type: KEY_CONTEXT_USER.HIDE_MODAL, payload: true });
        },
      },
  ...
  const handleDeleteItem = async (id) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/cart/deleteItemCart`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          data.map((cart, i) => (
            <div key={i} className="cart-item">
              ...
              <div className="btn-quantity">
                <button
                  ...
                  className={`btn-decrease ${
                    quantities[cart.productDetailCode] === 1
                ...
                <span
                  onClick={() => handleUpdateQuantity(cart.productDetailCode)}
                ...
                <button
                  onClick={() =>
                    increase(cart.productDetailCode, cart.quantity)
                  }
                  disabled={quantities[cart.productDetailCode] >= cart.quantity}
                  className={`btn-increase ${
                    quantities[cart.productDetailCode] >= cart.quantity
                      ? "opacity-50 cursor-not-allowed"
                      : ""
                  }`}
                >
              ...
              <span className="cart-price">
                Giá:<p>{formatter.format(cart.price)}</p>{" "}
              </span>
              <div className="icon-delete">
                <img
                  alt="xóa"
                  src={AppImages.deleteIcon}
                  onClick={() => handleDeleteItem(cart.productDetailCode)}
                />
              </div>
            </div>
          ))
