...
const ProductDetail = () => {
  ...
  const [riviewProduct, setRiviewProduct] = useState([]);
  ...
  const getProductById = async () => {
    try {
      const response = await fetch(
        `http://localhost:3001/product/getProductByIdForUser/${id}`,
        {
          headers: {
  ...
  const getProductByTrademark = async () => {
    try {
      const response = await fetch(
        `http://localhost:3001/product/getProductByTrademark`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer `,
    ...
    <div className="productDetail-container">
        ...
        <div className="productDetail-content">
          ...
          <div className="productDetail-right">
            ...
            <p>
              <b>Giá:</b>{" "}
              {formatter.format(
                filteredSizes.find((v) => v.size === selectedSize)?.price ||
                  "N/A"
              )}
            </p>
          ...
          <div className="riview">
            <h1>Đánh giá của khách hàng ({riviewProduct.length})</h1>
            {riviewProduct.map((v, i) => (
              <div className="item_riview">
                <div className="item_rate">
                  <strong>{v.customerName}</strong>
                  <p className="item_rate">
                    {v.sart}
                    <img src={AppImages.sart2} alt="" />
                  </p>
                </div>
                <p>{v.content}</p>
              </div>
      ...
      <div className="product-category">
        ...
        <div className="item-render">
          {dataTrademark.length > 0 ? (
            dataTrademark.map((v, i) => (
              <div key={i}>
