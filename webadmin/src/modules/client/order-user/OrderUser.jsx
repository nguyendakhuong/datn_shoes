...
const OrderUser = () => {
  ...
  const getAllOrder = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/order/getAllOrderByUser`,
        {
          headers: {
  ...
  const getOrderNote = async (orderCode) => {
    try {
      const response = await fetch(
        `http://localhost:3001/order/getOrderNote/${orderCode}`,
        {
          method: "GET",
          headers: {
    ...
    dispatch({
      ...
      payload: {
        ...
        onClickConfirmModel: async () => {
          try {
            const response = await fetch(
              `http://localhost:3001/order/cancelOrderUser`,
              {
                method: "POST",
                headers: {
        ...
        {sortedOrders.length > 0 ? (
          sortedOrders.map((order) => (
            <div
              ...
              onClick={() => handleClickItemOrderUser(order)}
            >
